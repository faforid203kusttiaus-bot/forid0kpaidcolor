...
<title>Unique Prediction Bot Panel</title>
<style>
  body {
...
<div class="main-container">
  ...
  <div class="box" id="latestHistoryBox">
    <h3>Latest History (Real Results)</h3>
...
<script>
  ...
  function updateBotHistory() {
    ...
    botPredictions.slice(-5).reverse().forEach(pred => { // Show last 5, oldest first
      const li = document.createElement('li');
      let outcomeHtml = '';
      if (pred.outcome) {
  ...
  async function generateSignal() {
    // First, fetch latest real history
    ...
    // Check for any pending outcomes
    checkOutcomes();
    ...
    if (botPredictions.length > 5) botPredictions.shift(); // Keep only last 5
